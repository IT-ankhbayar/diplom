  ...
  return (
    <Container>
      ...
      <div className="flex gap-4 mt-8 mb-8 justify-center">
        <button
          className={`px-4 py-2 rounded-lg font-semibold transition-colors duration-150 ${tab === 'trips' ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-blue-100'}`}
          onClick={() => setTab('trips')}
        >Миний аяллууд</button>
        <button
          className={`px-4 py-2 rounded-lg font-semibold transition-colors duration-150 ${tab === 'orders' ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-blue-100'}`}
          onClick={() => setTab('orders')}
        >Миний захиалгууд</button>
        <button
          className={`px-4 py-2 rounded-lg font-semibold transition-colors duration-150 ${tab === 'properties' ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-blue-100'}`}
